...
export default function FieldGuide({
  data,
}: Readonly<PageProps<Queries.FieldGuideTemplateQuery>>): JSX.Element {
  const species: PrintableSpecies[] = (data?.filteredSpecies || data?.allSpecies)?.edges.map((edge) => ({
    id: edge.node.id,
...
export const query = graphql`
  query FieldGuideTemplate ($location: String!, $useFilter: Boolean!) {
    filteredSpecies: allMarkdownRemark(
        filter: {
          frontmatter: { 
            templateKey: { eq: "species" } 
            locations: { in: [$location] }
          }
        }
    ) @include(if: $useFilter) {
      edges {
        node {
          id
          html
